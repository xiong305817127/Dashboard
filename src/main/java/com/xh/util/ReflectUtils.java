...
public class ReflectUtils {
	...
	/**
	 * 通过 父类或者接口 获取 所有的 实现子类
	 * @param parentClass
	 * @return
	 * @throws Exception
	 */
	public static <T> List<T> getImplementsByParentClass(Class<T> parentClass) throws Exception{
		List<T> result = new ArrayList<>();
		Set<Class<? extends T>> monitorClasses = new Reflections().getSubTypesOf(parentClass);
		for (Class<? extends T> t : monitorClasses) {
			result.add((T)t.cast(t.newInstance()));
		}
		return result ;
	}
