...
/**
 * An SFTP FileSystemConfigBuilder that uses Web variables to build SFTP VFS configuration options. Options can be
 ...
 * @author cboyden
 */
public class WebSftpFileSystemConfigBuilder extends WebGenericFileSystemConfigBuilder {

  private static final WebSftpFileSystemConfigBuilder builder = new WebSftpFileSystemConfigBuilder();
  ...
  public static WebSftpFileSystemConfigBuilder getInstance() {
    return builder;
  }

  protected WebSftpFileSystemConfigBuilder() {
    super();
  }
    ...
    if ( !fullParameterName.startsWith( "vfs.sftp" ) ) {
      ...
      try {
        ...
        if ( !parameterContainsHost( fullParameterName ) || fullParameterName.endsWith( file.getHostName() ) ) {
          ...
          // No host match found
          logger.debug( "No host match found for: " + fullParameterName );
        }
      } catch ( IOException e ) {
    	  logger.error( "Failed to set VFS parameter: [" + fullParameterName + "] " + value, e );
      }
    }
  }
